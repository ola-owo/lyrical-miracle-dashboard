...
df_search_results = (
    ...
    .with_columns(normalize_song_titles(
...
titles_mat = TfidfVectorizer(strip_accents='unicode').fit(
    pl.concat((df_search_results['full_title'],
               df_search_results['g_full_title'])))
titles_strip_mat = TfidfVectorizer(strip_accents='unicode').fit(
    pl.concat((df_search_results['full_title_strip'],
               df_search_results['g_full_title_strip'])))
artists_mat = TfidfVectorizer(strip_accents='unicode').fit(
    pl.concat((df_search_results['artist_strip'],
               df_search_results['g_artist_strip'])))
songs_mat = TfidfVectorizer(strip_accents='unicode').fit(
    pl.concat((df_search_results['song_strip'],
               df_search_results['g_song_strip'])))
...
def text_similarities(df: pl.DataFrame) -> pl.DataFrame:
    ...
    cosine_sims = {name: np.diag(cosine_similarity(tfidf.transform(df[c1]), tfidf.transform(df[c2])))
...
df_search_results_scored = (
    df_search_results
    .join(df_song_matches_old, on='id', how='anti')
    .group_by('id')
...
df_search_results_filtered = (
    ...
    .filter(
        ...
        pl.col('cos') == pl.col('cos').max().over('id')
        )
